...
- **Message store** — `LiteInboxStore` backed by `svrn7-msg.db`. `InboundMessage` stores `FromDid` (sender DID) and `WireId` (DIDComm wire `id`; null for encrypted messages), both threaded from `DrawbridgeService` via `IInboxStore.EnqueueAsync`. Inbox dequeue/dispatch is owned exclusively by `DIDCommMessageSwitchboard` (in the TDA host) — nothing in `Svrn7.Society` reads from the inbox.
